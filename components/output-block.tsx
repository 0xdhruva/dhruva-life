...
export function OutputBlock({ id, timestamp, command, content, type = "command" }: OutputBlockProps) {
  ...
  useEffect(() => {
    // Check if content is long (for mobile pagination)
  ...
  return (
    <div className="group space-baseline-2" id={id}>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity flex space-x-2">
          <Button
            ...
            size="sm"
            onClick={handleCopy}
            className="h-7 px-3 text-xs text-sage hover:text-bone hover:bg-sage/20"
          >
            {copied ? "Copied!" : "Copy"}
          </Button>
          <Button
      ...
      <div className={`${typeColors[type]} ${isAnimating ? "animate-pulse" : ""} content-width space-baseline`}>
        <div className="px-6 py-4 bg-sage/5 rounded-md">
          {isLongContent && !showMore ? (
            <div>
              ...
              <Button
                ...
                className="mt-4 text-terracotta hover:text-bone hover:bg-terracotta/20"
              >
                Show more ↓
              </Button>
            </div>
      ...
      <div className="mt-6 text-center text-sage/30 text-sm space-baseline">
        <div className="flex items-center justify-center space-x-2">
          <div className="w-8 h-px bg-sage/20"></div>
          <span>◦</span>
          <div className="w-8 h-px bg-sage/20"></div>
        </div>
      </div>
